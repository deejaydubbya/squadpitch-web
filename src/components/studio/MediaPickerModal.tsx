...
export function MediaPickerModal({
  ...
  return (
    <div
      className="mobile-dialog-backdrop fixed inset-0 z-50 flex justify-center bg-black/60"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
      role="dialog"
      aria-modal="true"
      aria-label={mode === 'multi' ? 'Choose images' : 'Choose an image'}
    >
      <div className="mobile-dialog-surface flex w-full max-w-3xl flex-col rounded-t-2xl border border-white-10 bg-sp-card shadow-2xl sm:rounded-2xl">
