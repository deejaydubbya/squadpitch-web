    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-white-40 mt-0.5">
          Connect your business accounts to automatically import reviews, deals,
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          >
            <Users className={cn('w-5 h-5', isConnected ? 'text-accent-green-110' : 'text-white-40')} />
          </div>
          <div>
            <h4 className="text-sm font-semibold text-white-100">
              CRM — Follow Up Boss
            ...
            <p className="text-xs text-white-40">
              Import deals, contacts, and client feedback
            </p>
          </div>
        </div>
      ...
      {isConnected && (
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          {(status?.dealCount ?? 0) > 0 && (
            <span className="text-white-40">
              {status?.dealCount} deals
            </span>
          )}
          {(status?.contactCount ?? 0) > 0 && (
            <span className="text-white-40">
              {status?.contactCount} contacts
            </span>
          )}
          {status?.lastSyncedAt && (
            <span className="text-white-30">
