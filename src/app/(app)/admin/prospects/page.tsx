...
export default function AgentOutreachPage() {
  ...
  const visible = useMemo(() => { const base = tab === "Discover Agents" ? discoveryGroups[discoveryView] : prospects.filter((p) => (tab === "Preview Queue" ? ["QUALIFIED", "PREVIEW_PENDING", "PREVIEW_GENERATING", "PREVIEW_FAILED"].includes(p.status) : tab === "Ready for Email" ? ["READY_TO_EMAIL", "EMAIL_FAILED"].includes(p.status) : ["EMAIL_QUEUED", "EMAIL_SENDING", "EMAIL_SENT", "UNCLAIMED", "CLAIMED", "BOUNCED", "UNSUBSCRIBED", "EMAIL_FAILED"].includes(p.status))); if(tab!=="Outreach"||funnelFilter==="all")return base; return base.filter(p=>({never_opened:!!p.emailSentAt&&!p.emailFirstOpenedAt,opened_not_clicked:!!p.emailFirstOpenedAt&&!p.claimLinkFirstClickedAt,clicked_not_previewed:!!p.claimLinkFirstClickedAt&&!p.previewFirstViewedAt,previewed_not_started:!!p.previewFirstViewedAt&&!p.claimStartedAt,started_not_claimed:!!p.claimStartedAt&&!p.claimedAt,claimed:!!p.claimedAt,unsubscribed:!!p.unsubscribedAt,bounced:!!p.bouncedAt,delivered:!!p.emailDeliveredAt,delivery_unknown:!!p.emailSentAt&&!p.emailDeliveredAt&&!p.bouncedAt}[funnelFilter])); }, [discoveryGroups, discoveryView, prospects, tab, funnelFilter]);
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "Outreach" && data?.funnel && <><div className="grid grid-cols-2 gap-3 md:grid-cols-5">{[["Sent",data.funnel.sent],["Delivered",data.funnel.deliveryConfirmed],["Delivery unknown",data.funnel.deliveryUnknown],["Estimated opened",data.funnel.uniqueEstimatedOpened],["Likely-human clicked",data.funnel.likelyHumanUniqueClicked],["Preview viewed",data.funnel.uniquePreviewViewed],["Claim started",data.funnel.uniqueClaimStarted],["Claimed",data.funnel.claimed],["Unsubscribed",data.funnel.unsubscribed],["Bounced",data.funnel.bounced]].map(([label,value])=><div key={String(label)} className="rounded-xl border border-white-10 bg-sp-card p-3"><div className="text-xs text-white-40">{label}</div><div className="mt-1 text-xl font-semibold text-white">{value}</div></div>)}</div><div className="flex flex-wrap items-center gap-3"><select value={funnelFilter} onChange={e=>setFunnelFilter(e.target.value)} className="min-h-11 rounded-lg border border-white-10 bg-sp-bg px-3 text-white"><option value="all">All outreach</option><option value="never_opened">Sent, never opened</option><option value="opened_not_clicked">Opened, never clicked</option><option value="clicked_not_previewed">Clicked, never previewed</option><option value="previewed_not_started">Previewed, claim not started</option><option value="started_not_claimed">Claim started, not claimed</option><option value="claimed">Claimed</option><option value="unsubscribed">Unsubscribed</option><option value="bounced">Bounced</option><option value="delivered">Delivery confirmed</option><option value="delivery_unknown">Delivery unknown</option></select><p className="text-xs text-white-40">Opens are estimated; privacy proxies and scanners can inflate engagement. Generic SMTP delivery remains unknown.</p></div></>}
            ...
            rows.map((row) => (
              <tr key={row.id} className="border-t border-white-10">
                ...
                <td className="p-3">
                  ...
                  {row.emailSentAt && <details className="mt-2"><summary className="cursor-pointer text-xs text-white-50">Funnel details</summary><div className="mt-2 flex max-w-72 flex-wrap gap-1 text-[11px]">{[["Sent",row.emailSentAt?"✓":"—"],["Delivered",row.emailDeliveredAt?"✓":"?"],["Opened",row.emailOpenCount||"—"],["Clicked",row.claimLinkClickCount||"—"],["Previewed",row.previewViewCount||"—"],["Claim started",row.claimStartedAt?"✓":"—"],["Claimed",row.claimedAt?"✓":"—"]].map(([label,value])=><span key={String(label)} className="rounded bg-white-5 px-2 py-1" title={String(label)}>{label} {value}</span>)}</div><ol className="mt-2 space-y-1 text-xs text-white-50">{[...row.events].reverse().map(event=><li key={event.id}>{new Date(event.createdAt).toLocaleString()} — {event.type.replaceAll("_"," ").toLowerCase()}</li>)}</ol></details>}
